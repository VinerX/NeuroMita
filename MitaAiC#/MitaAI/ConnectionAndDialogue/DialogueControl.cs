...
namespace MitaAI
{
    static class DialogueControl
    {
        ...
        private static IEnumerator DisplayResponseAndEmotion(characterType characterToSend, string response, string patch_to_sound, AudioSource audioSource = null)
        {
        ...
        public static IEnumerator ShowDialoguesSequentially(characterType characterToSend,List<string> dialogueParts, bool itIsWaitingDialogue)
        {
        ...
        public static IEnumerator PlayerTalk(string text)
        {
            ...
            if (currentDialog != null)
            {
                ...
                try
                {
                    ...
                    addDialogueMemory(answer,characterType.Player);
        ...
        private static IEnumerator ShowDialogue(characterType characterToSend, string part, float delay, bool itIsWaitingDialogue = false)
        {
        ...
        static void changeTextColor(GameObject currentDialog,characterType characterToSend)
        {
        ...
        private static void addDialogueMemory(Dialogue_3DText dialogue_3DText,characterType speaker)
        {
        ...
        // ---------------------------------------------------------------
        // Structured Output path — processes pre-parsed segments directly
        // ---------------------------------------------------------------

        public static IEnumerator DisplayStructuredResponseCoroutine(int id, characterType characterToSend, StructuredResponse structured)
        {
            while (dialogActive) { yield return null; }
            dialogActive = true;

            MelonLogger.Msg($"DisplayStructuredResponseCoroutine: {structured.segments.Count} segment(s)");
            yield return null;

            // Voice: use concatenated response text for TTS
            string fullText = structured.response;
            if (string.IsNullOrEmpty(fullText))
            {
                var sb = new StringBuilder();
                foreach (var seg in structured.segments)
                {
            ...
            // Wait for voice audio (same logic as legacy path)
            string patch_to_sound = "";
            ...
            float totalDelay = fullText.Length / simbolsPerSecond;
            MelonCoroutines.Start(PlayMitaSound(totalDelay, audioClip, fullText.Length));
            ...
            foreach (var seg in structured.segments)
            {
                yield return MelonCoroutines.Start(ShowStructuredSegment(characterToSend, seg));
            }
        ...
        private static IEnumerator ShowStructuredSegment(characterType characterToSend, ResponseSegment seg)
        {
            EmotionType emotion = EmotionType.none;
            ...
            {
                // Face params
                foreach (var fp in seg.face_params)
                {
                    MitaFaceAnimationModded.SetFaceStyle("<f>" + fp + "</f>");
                }

                // Clothes
                foreach (var cloth in seg.clothes)
                {
                    MitaClothesModded.ProcessClothes("<clothes>" + cloth + "</clothes>");
                }

                // Visual effects
                foreach (var vfx in seg.visual_effects)
                {
                    PlayerEffectsModded.ProcessPlayerEffects("<v>" + vfx + "</v>");
                }

                // Animations
                var animModded = MitaAnimationModded.getMitaAnimationModded(characterToSend);
                foreach (var anim in seg.animations)
                {
                    animModded.setAnimation("<a>" + anim + "</a>");
                }

                // Movement modes
                foreach (var mm in seg.movement_modes)
                {
                    MitaMovement.FindMovementStyle(characterToSend, "<m>" + mm + "</m>");
                }

                // Music
                foreach (var m in seg.music)
                {
                    AudioControl.ProcessMusic("<music>" + m + "</music>");
                }

                // Hint
                if (!string.IsNullOrEmpty(seg.hint))
                {
                    CommandProcessor.ProcesHint("<hint>" + seg.hint + "</hint>");
                }

                // Interactions
                foreach (var interaction in seg.interactions)
                {
                    ObjectAnimationMita.ProcessInteraction("<interaction>" + interaction + "</interaction>");
                }

                // Emotions (apply last so they show during text)
                foreach (var emo in seg.emotions)
                {
                    if (Enum.TryParse<EmotionType>(emo, true, out var parsedEmotion))
                    {
                        emotion = parsedEmotion;
                        MitaCore.Instance.Mita.FaceEmotionType(emotion);
                    }
                }

                // Commands
                if (seg.commands.Count > 0)
                {
                    CommandProcessor.ProcessCommands(seg.commands);
                }
            }
            catch (Exception ex)
            {
                MelonLogger.Error($"Error processing structured segment: {ex.Message}");
            }
