...
#### Основные
   ...
   - Бесплатные API:  
     ...
     - gpt4free https://g4f.dev/members.html - API или отдельная галочка без ключа. Лимиты - 300к токенов в час, 100 запросов в час и 1млн токенов в день.
   ...
   Внимание, чаще всего модели gemini лучше справляются с передачей эмоций, так что обычно интересней играть с ними. DeepSeek чуть хуже (но умнее обрабатывает команды).
...
### Модели для генерации (на 13.03.2026)
   ...
   - gpt4free всё ещё обновляется и поддерживается
...
### G4F (Последняя версия - 7.3.0 от xtekky)
...
Ссылка https://api.intelligence.io.solutions/api/v1/
  ...
  - Есть 1 рабочий способ: Создаём на их сайте по несколько аккаунтов, на каждом создаём API ключи, затем вставляем их в поле "Резервные ключи" (для опенроутера или google ai способ также может работать).  
...
В ссылку после models/ вставлять нужную модель (примерно так: models/gemini-3.1-flash-lite-preview или models/gemini-3.0-pro).  
