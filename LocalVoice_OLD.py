...
class LocalVoice:
    def __init__(self, parent=None):
        ...
        try:
            ...
            logger.info(f"Модуль f5_tts не установлен: {ex}")
    ...
    def uninstall_f5_tts(self):
        return self._uninstall_component("F5-TTS", "f5-tts")
    ...
    def _cleanup_after_uninstall(self, removed_package_name: str):
        ...
        if removed_package_name == "tts-with-rvc":
            ...
            if self.current_model == "f5_tts":
                self.current_model = None
            if "f5_tts" in self.initialized_models:
                self.initialized_models.remove("f5_tts")
            if self.current_f5_pipeline:
                self.current_f5_pipeline = None
            self.f5_pipeline_module = None
            model_id_to_clear = "f5_tts"
    ...
    def download_f5_tts(self):
        """Загружает и устанавливает модель F5-TTS и ее зависимости."""
        gui_elements = None
        try:
            gui_elements = self._create_installation_window(
            ...
            # Шаг 1: Установка PyTorch с CUDA, если нужно
            if self.provider in ["NVIDIA"] and not self.is_cuda_available():
                update_status(_("Установка PyTorch с поддержкой CUDA 12.4...", "Installing PyTorch with CUDA 12.4 support..."))
                update_progress(10)
                success = installer.install_package(
            ...
            # Шаг 2: Установка библиотеки f5-tts
            update_status(_("Установка f5-tts и зависимостей...", "Installing f5-tts and dependencies..."))
            success = installer.install_package(
                ["f5-tts", "google-api-core"],  # <-- ИЗМЕНЕНИЕ ЗДЕСЬ
                description=_("Установка f5-tts и google-api-core...", "Installing f5-tts and google-api-core...")
            )
            if not success:
                update_status(_("Ошибка при установке f5-tts", "Error installing f5-tts"))
                if progress_window and progress_window.winfo_exists():
                    progress_window.after(5000, progress_window.destroy)
                return False
            update_progress(50)

            # Шаг 3: Загрузка файлов модели
            update_status(_("Загрузка файлов модели F5-TTS...", "Downloading F5-TTS model files..."))
            
            # Вспомогательная функция для скачивания с прогрессом
            def _download_file_with_progress(url, dest_path, file_description):
                import requests
                try:
                    update_log(_(f"Начинаю загрузку: {file_description}", f"Starting download: {file_description}"))
                    update_log(f"URL: {url}")
                    update_log(_(f"Путь назначения: {dest_path}", f"Destination: {dest_path}"))
                    ...
                    with open(dest_path, 'wb') as f:
                        for chunk in response.iter_content(chunk_size=block_size):
                            ...
                            if total_size > 0:
                                ...
                                update_status(_(f"Загрузка {file_description}: {int(percent)}%", f"Downloading {file_description}: {int(percent)}%"))
                    
                    update_log(_(f"Файл {file_description} успешно загружен.", f"File {file_description} downloaded successfully."))
                    return True
                except requests.exceptions.RequestException as e:
                    update_log(_(f"Ошибка при загрузке {file_description}: {e}", f"Error downloading {file_description}: {e}"))
                    return False

            model_dir = os.path.join("checkpoints", "F5-TTS")
            os.makedirs(model_dir, exist_ok=True)
            update_log(_(f"Создана директория: {model_dir}", f"Created directory: {model_dir}"))
            ...
            model_path = os.path.join(model_dir, "model_240000_inference.safetensors")
            
            vocab_url = "https://huggingface.co/Misha24-10/F5-TTS_RUSSIAN/resolve/main/F5TTS_v1_Base/vocab.txt?download=true"
            vocab_path = os.path.join(model_dir, "vocab.txt")

            # Загружаем модель
            if not _download_file_with_progress(model_url, model_path, "model.safetensors"):
                update_status(_("Ошибка загрузки файла модели", "Model file download error"))
                return False
            update_progress(75)

            # Загружаем словарь
            if not _download_file_with_progress(vocab_url, vocab_path, "vocab.txt"):
                update_status(_("Ошибка загрузки файла словаря", "Vocab file download error"))
                return False
            update_progress(90)

            # Шаг 4: Попытка импорта
            update_status(_("Попытка импорта модуля...", "Attempting to import module..."))
            try:
                from LocalPipelines.F5_TTS.f5_pipeline import F5TTSPipeline
                self.f5_pipeline_module = F5TTSPipeline
                update_log(_("Модуль F5-TTS успешно загружен!", "F5-TTS module loaded successfully!"))
            except ImportError as ie:
                update_log(_(f"Предупреждение: Модуль установлен, но не может быть загружен немедленно: {ie}", f"Warning: Module installed, but cannot be loaded immediately: {ie}"))
            ...
            self.current_model = "f5_tts"
            return True

        except Exception as e:
            logger.error(_(f"Критическая ошибка при установке F5-TTS: {e}", f"Critical error during F5-TTS installation: {e}"))
            traceback.print_exc()
            if gui_elements and gui_elements["window"] and gui_elements["window"].winfo_exists():
                gui_elements["update_status"](_("Критическая ошибка установки!", "Critical installation error!"))
                gui_elements["window"].after(5000, gui_elements["window"].destroy)
    ...
    async def voiceover(self, text, output_file="output.wav", character=None):
        ...
        elif self.current_model == "f5_tts":
            return await self.voiceover_f5_tts(text, character)
    ...
    async def voiceover_edge_tts_rvc(self, text, TEST_WITH_DONE_AUDIO: str = None):
        ...
        try:
            ...
            # --- ИСПРАВЛЕНИЕ: Добавлена проверка на None и существование файла ---
            if use_index_file and self.index_path and os.path.exists(self.index_path):
                ...
                if use_index_file and self.index_path: # Логируем, почему не используем, если должны были
                    logger.info(f"Не используем индексную базу для RVC (файл не найден: {self.index_path}).")
    ...
    async def voiceover_f5_tts(self, text, character=None, reference_postfix="default"):
        """
        Создает озвучку текста с использованием F5-TTS, используя специфичный для персонажа референс.
        
        :param text: Текст для озвучки.
        :param character: Объект персонажа, содержащий short_name.
        :param reference_postfix: Постфикс для файла референса (например, "default", "angry", "happy").
        """
        if not self.current_f5_pipeline:
            logger.error("Пайплайн F5-TTS не инициализирован. Попытка инициализации...")
            if not self.initialize_model("f5_tts"):
                 raise Exception("Не удалось инициализировать модель F5-TTS")

        try:
            settings = self.load_model_settings("f5_tts")
            speed = float(settings.get("speed", 1.0))
            nfe_step = int(settings.get("nfe_step", 32))
            remove_silence = settings.get("remove_silence", True)

            # --- Новая логика для поиска референса ---
            ref_audio_path = None
            ref_text_content = ""
            
            # 1. Пытаемся найти специфичный референс для персонажа
            if character and hasattr(character, 'short_name'):
                char_name = character.short_name
                ref_folder = os.path.join("Models", f"{char_name}_Cuts")
                ref_basename = f"{char_name}_{reference_postfix}"
                
                potential_audio_path = os.path.join(ref_folder, f"{ref_basename}.wav")
                potential_text_path = os.path.join(ref_folder, f"{ref_basename}.txt")

                if os.path.exists(potential_audio_path):
                    ref_audio_path = potential_audio_path
                    logger.info(f"Найден специфичный референс для F5-TTS: {ref_audio_path}")
                    ...
                    if os.path.exists(potential_text_path):
                        ...
                        logger.warning(f"Аудио референс найден, но текстовый файл отсутствует: {potential_text_path}. Генерация может быть неточной.")
                        # F5-TTS требует текст, поэтому предоставляем заглушку
                        ref_text_content = "Это пример текста для референсного аудио."
                else:
                    logger.warning(f"Специфичный референс для '{char_name}' не найден по пути: {potential_audio_path}. Поиск референса по умолчанию...")
            
            # 2. Если специфичный референс не найден, используем референс по умолчанию
            if not ref_audio_path:
                ...
                if os.path.exists(default_audio_path):
                    ...
                    logger.info(f"Используется референс по умолчанию: {ref_audio_path}")
                    ...
                    # Это критическая ошибка, так как модель не может работать без референса
                    logger.error("Критическая ошибка: Не найден ни специфичный, ни дефолтный референс для F5-TTS.")
            ...
            filename = f"f5_raw_{hash_object.hexdigest()[:10]}.wav"
            output_path = os.path.join(temp_dir, filename)

            logger.info(f"Начинаем генерацию F5-TTS для текста: '{text[:50]}...'")
            ...
            if not os.path.exists(output_path) or os.path.getsize(output_path) == 0:
                logger.error(f"F5-TTS не сгенерировал аудиофайл или файл пуст: {output_path}")
                return None

            logger.info(f"F5-TTS сгенерировал аудио: {output_path}")

            # --- ИСПРАВЛЕННАЯ ЛОГИКА КОНВЕРТАЦИИ И УДАЛЕНИЯ ---

            # 1. Надежно создаем имя для стерео-файла
            base_path, extension = os.path.splitext(output_path)
            stereo_output_path = base_path.replace("_raw", "_stereo") + extension
            
            # 2. Вызываем конвертацию с новым, уникальным именем файла
            converted_file = await self.convert_wav_to_stereo(output_path, stereo_output_path)

            # 3. Четкая логика выбора итогового файла
            if converted_file and os.path.exists(converted_file):
                logger.info(f"Файл успешно конвертирован в стерео: {stereo_output_path}")
                # Если конвертация прошла успешно, итоговый файл - это новая стерео-версия
                final_output_path = stereo_output_path
                # Удаляем старый моно-файл
                try:
                    os.remove(output_path)
                    logger.info(f"Удален промежуточный файл: {output_path}")
                except OSError as e:
                    logger.warning(f"Не удалось удалить промежуточный файл {output_path}: {e}")
            else:
                logger.warning("Не удалось конвертировать файл в стерео, используется исходный.")
                # Если конвертация не удалась, итоговый файл - это исходная моно-версия
                final_output_path = output_path
            
            # --- КОНЕЦ ИСПРАВЛЕНИЙ ---

            if self.parent.ConnectedToGame:
                self.parent.patch_to_sound_file = final_output_path
    ...
    def is_model_installed(self, model_id):
        ...
        if model_id == "low":
            ...
            return self.is_f5_tts_installed()
        elif model_id == "high":
    ...
    def is_f5_tts_installed(self):
        """Проверяет, установлен ли Fish Speech"""
        return self.f5_pipeline_module is not None
    ...
    def initialize_model(self, model_id, init=False):
        """Инициализирует модель с указанным ID и параметрами из настроек, используя готовые функции озвучки для 'прогрева'."""
        if model_id not in ["low", "low+", "medium", "medium+", "medium+low", "f5_tts"]:
            logger.info(f"Неизвестный ID модели: {model_id}")
            return False

        if self.is_model_initialized(model_id):
            logger.info(f"Модель {model_id} уже инициализирована, пропуск.")
            return True

        # --- ИСПРАВЛЕНИЕ 1: Устанавливаем текущую модель в самом начале ---
        # Это решает проблему с `load_model_settings(None)`
        self.current_model = model_id
        
        settings = self.load_model_settings(model_id)
        if not settings:
            logger.warning(f"Не найдены настройки для модели {model_id}. Используются значения по умолчанию.")

        try:
            # --- Блок инициализации Edge-TTS + RVC (для 'low' и как компонент для 'low+' и 'medium+low') ---
            if model_id in ["low", "low+", "medium+low"]:
                if self.tts_rvc_module is None:
                    logger.error(f"Модуль tts_with_rvc не установлен, но требуется для модели {model_id}")
                    return False

                if self.current_tts_rvc is None:
                    device_key = "fsprvc_rvc_device" if model_id == "medium+low" else "device"
                    f0_method_key = "fsprvc_f0method" if model_id == "medium+low" else "f0method"
                    
                    device = settings.get(device_key, "cuda:0" if self.provider == "NVIDIA" else "dml")
                    f0_method = settings.get(f0_method_key, "rmvpe" if self.provider == "NVIDIA" else "pm")
                    
                    is_nvidia = self.provider in ["NVIDIA"]
                    model_ext = 'pth' if is_nvidia else 'onnx'
                    default_model_path = os.path.join("Models", f"Mila.{model_ext}")
                    
                    model_path_to_use = self.pth_path if self.pth_path and os.path.exists(self.pth_path) else default_model_path
                    ...
                    if self.voice_language == "ru":
                        ...
                        self.current_tts_rvc.set_voice("en-US-AvaMultilingualNeural")
                    logger.info(f"Компонент RVC для модели '{model_id}' инициализирован.")

            # --- Блок инициализации Silero (для 'low+') ---
            if model_id == "low+":
                if self.current_silero_model is None:
                    try:
                        ...
                        language = 'en' if self.voice_language == 'en' else 'ru'
                        ...
                        logger.info(f"Компонент Silero для low+ инициализирован на устройстве {silero_device}.")
                    except Exception as e:
                        logger.error(f"Ошибка инициализации компонента Silero для low+: {e}", exc_info=True)
                        return False

            # --- Блок инициализации Fish Speech (для 'medium', 'medium+', 'medium+low') ---
            if model_id in ["medium", "medium+", "medium+low"]:
                if self.fish_speech_module is None:
                    logger.error(f"Модуль fish_speech_lib не установлен, но требуется для модели {model_id}")
                    return False
                
                if self.current_fish_speech is None:
                    compile_model = model_id in ["medium+", "medium+low"]
                    device = settings.get("fsprvc_fsp_device" if model_id == "medium+low" else "device", "cuda")
                    half = settings.get("fsprvc_fsp_half" if model_id == "medium+low" else "half", "True" if compile_model else "False").lower() == "true"

                    self.current_fish_speech = self.fish_speech_module(device=device, half=half, compile_model=compile_model)
                    if compile_model and not self.first_compiled:
                        self.first_compiled = True
                    logger.info(f"Компонент Fish Speech для модели '{model_id}' инициализирован (compile={compile_model}).")

            # --- Блок инициализации F5-TTS ---
            if model_id == "f5_tts":
                ...
                if self.current_f5_pipeline is None:
                    ...
                    device = "cuda" if self.provider == "NVIDIA" else "cpu"
            ...
            # --- Тестовый прогон для "прогрева" модели, если init=True ---
            if init:
                # --- ИСПРАВЛЕНИЕ 2: Устанавливаем пути к моделям по умолчанию ПЕРЕД вызовом voiceover_* ---
                # Это решает проблему с `os.path.abspath(None)`
                is_nvidia = self.provider in ["NVIDIA"]
                model_ext = 'pth' if is_nvidia else 'onnx'
                self.pth_path = os.path.join("Models", f"Mila.{model_ext}")
                self.index_path = os.path.join("Models", "Mila.index")
                self.clone_voice_filename = os.path.join("Models", "Mila.wav")
                self.clone_voice_text = os.path.join("Models", "Mila.txt")
                self.current_character_name = "Mila" # Устанавливаем имя по умолчанию для корректных настроек
                
                init_text = f"Инициализация модели {model_id}" if self.voice_language == "ru" else f"{model_id} Model Initialization"
                logger.info(f"Выполнение тестового прогона для {model_id}...")
                
                loop = asyncio.new_event_loop()
                asyncio.set_event_loop(loop)
                try:
                    # Выбираем и вызываем соответствующую функцию озвучки
                    if model_id == "low":
                        loop.run_until_complete(self.voiceover_edge_tts_rvc(init_text))
                    elif model_id == "low+":
                        loop.run_until_complete(self.voiceover_silero_rvc(init_text))
                    elif model_id in ["medium", "medium+", "medium+low"]:
                        compile_flag = model_id in ["medium+", "medium+low"]
                        rvc_flag = model_id == "medium+low"
                        loop.run_until_complete(self.voiceover_fish_speech(text=init_text, compile=compile_flag, with_rvc=rvc_flag))
                    elif model_id == "f5_tts":
                        loop.run_until_complete(self.voiceover_f5_tts(init_text))
                    
                    logger.info(f"Тестовый прогон для {model_id} успешно завершен.")
                except Exception as e:
                    logger.error(f"Ошибка во время тестового прогона модели {model_id}: {e}", exc_info=True)
                    return False # --- ИСПРАВЛЕНИЕ 3: Возвращаем False при ошибке в тестовом прогоне
                finally:
                    loop.close()
            
            self.add_to_initialized(model_id)
            logger.info(f"Модель {model_id} успешно инициализирована и готова к использованию.")
            return True

        except Exception as e:
            logger.error(f"Критическая ошибка при инициализации модели {model_id}: {e}", exc_info=True)
            # Сбрасываем инстансы и текущую модель в случае ошибки
            self.current_model = None
            self.current_tts_rvc = None
            self.current_silero_model = None
            self.current_fish_speech = None
