    ...
    MODEL_CONFIGS = [
        {
            ...
            "pipeline": "edge",
            "name": "Edge-TTS + RVC (ONNX/DirectML)",
            "min_vram": 3,
            "rec_vram": 4,
            "gpu_vendor": ["AMD", "CPU"],
            "size_gb": 3,
            "languages": ["Russian", "English"],
            "intents": [_("ONNX", "ONNX"), _("DirectML", "DirectML"), _("Стабильно", "Stable")],
            "description": _("Edge-TTS с RVC через ONNX/DirectML. Ветка для AMD, Intel и CPU без fp16.", "Edge-TTS with RVC via ONNX/DirectML. Variant for AMD, Intel and CPU without fp16."),
            "settings": _onnx_edge_settings(),
        },
        {
            "id": SILERO_RVC_ONNX_ID,
            "pipeline": "silero",
            "name": "Silero + RVC (ONNX/DirectML)",
            "min_vram": 3,
            "rec_vram": 4,
            "gpu_vendor": ["AMD", "CPU"],
            "size_gb": 3,
            "languages": ["Russian", "English"],
            "intents": [_("ONNX", "ONNX"), _("DirectML", "DirectML"), _("Локальный синтез", "Offline synth")],
            "description": _("Silero + RVC через ONNX/DirectML с ограниченными стабильными параметрами.", "Silero + RVC through ONNX/DirectML with a limited stable parameter set."),
