"""
MessageWidget — individual chat message displayed as a QFrame card.

Supports user, assistant, system, and think message types with avatars,
colored labels, and optional structured output panels.
"""
...
# ── Avatar paths by character short_name ────────────────────────────────────
# Put avatar PNGs here (64x64 recommended). Placeholder used when missing.
AVATAR_DIR = os.path.join("assets", "avatars")

# Mapping: character name -> avatar filename
AVATAR_MAP = {
...
def _placeholder_avatar(size: int, color: str) -> QPixmap:
    """Generate a colored circle placeholder avatar."""
    ...
    painter.drawText(pm.rect(), Qt.AlignmentFlag.AlignCenter, "M")
    painter.end()
    return pm


def _get_avatar_pixmap(character_name: str, role: str) -> QPixmap:
    """Load avatar from file or return placeholder."""
    ...
    return _placeholder_avatar(AVATAR_SIZE, color)


class MessageWidget(QFrame):
    """
    A chat message card with optional avatar, speaker label, and content area.

    Layout:
    ┌─────────────────────────────────────────┐
    │ [avatar]  SpeakerName: message text ...  │
    ...
    def __init__(
        ...
        if show_avatar and role in ("assistant", "user"):
            ...
            main_layout.addWidget(self._avatar_label, 0, Qt.AlignmentFlag.AlignTop)
        else:
        ...
        # Placeholder for structured output panel (added later)
        ...
        self._content_layout.addLayout(self._structured_container)
    ...
    def set_speaker_name(self, name: str):
        ...
        color = ROLE_COLORS.get(self._role, "#dcdcdc")
    ...
    def add_structured_widget(self, widget: QWidget):
        """Add a structured output panel below the message text."""
...
class ThinkBlockWidget(QFrame):
    ...
    Shows a clickable header that toggles content visibility.
    """
    ...
    def toggle(self):
        ...
        verb = "думала"
        self._header.setText(f"{arrow} {self._speaker_name} {verb}...")
