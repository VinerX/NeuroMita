...
# Path to the DSL script that generates the structured-output format instruction.
# Resolved relative to the character's base_data_path via "../../Structural/..." so
# it lands in Prompts/Structural/ regardless of which character/variant is active.
_STRUCTURED_OUTPUT_SCRIPT_REL = "../../Structural/response_format_json.script"
...
class PromptController:
    ...
    def _build_system_messages(
        ...
        # The instruction is generated by a DSL script so it can branch on settings
        # (e.g. GRAPH_EXTRACTION_ENABLED) without touching Python code.
        caps = capabilities or {}
        if caps.get("structured_output", False):
            so_prompt = self._load_structured_output_prompt(character, caps)
            if so_prompt:
    ...
    def _load_structured_output_prompt(self, character, caps: dict) -> str:
        """Generate the structured-output format instruction via DSL script.

        The script (response_format_json.script) lives in Prompts/Structural/ and
        has access to character.app_vars (including GRAPH_EXTRACTION_ENABLED), so it
        can conditionally include the entities extraction section without any Python logic.

        The {TOOLS_DESCRIPTION} placeholder in the script output is replaced here with
        the actual tools prompt text from capabilities.
        """
        so_prompt = ""
        try:
            if hasattr(character, "dsl_interpreter") and character.dsl_interpreter:
                ...
                logger.warning("[PromptController] Character has no dsl_interpreter; structured output prompt skipped")
        except Exception as e:
            logger.warning(f"[PromptController] Failed to load structured output prompt via DSL: {e}")
